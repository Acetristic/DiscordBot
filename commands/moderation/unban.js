...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!args[0])
      return message.channel.send(
        "Please give me a user ID that you want me to unban."
      );

    if (isNaN(args[0])) return message.channel.send("Next time, get a valid ID.");

    if (args[0] === message.author.id)
      return message.channel.send("Um, why would you unban yourself?");

    if (args[0] === message.guild.owner.user.id)
      return message.channel.send("The server owner is already unbanned shut up."
      );

    if (args[0] === client.user.id)
      return message.channel.send("I am unbanned big noob biggest noob.");
    ...
    if (!Member)
      return message.channel.send(
        "Give a valid ID, or maybe given member is already unbanned."
      );
